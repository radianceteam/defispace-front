        ...
        <MainBlock
            ...
            content={
              <>
                ...
                <div className="confirm-block" style={{display:"flex",flexDirection:"column"}}>
                  <div className="returnViewDataItem">
                    <span className="confirm-value">~{props.qtyA.toFixed(4)}</span>
                    <div className="displayFlex">
                    <img className="confirm-icon" src={iconGenerator(props.fromToken.symbol)} alt={props.fromToken.symbol}/>
                    <span className="confirm-tokenF" style={{fontSize:"22px"}}>{props.fromToken.symbol}</span>
                    </div>
                  </div>
                  <div className="returnViewDataItem">
                    <span className="confirm-value">~{props.qtyB.toFixed(4)}</span>
                    <div className="displayFlex">
                    <img className="confirm-icon" src={iconGenerator(props.toToken.symbol)} alt={props.toToken.symbol}/>
                    <span className="confirm-tokenF" style={{fontSize:"22px"}}>{props.toToken.symbol}</span>
                    </div>
                  </div>

                </div>
                {/*<p className="confirm-text">Output is estimated. If the price changes by more than 0.5% your transaction will revert</p>*/}
                <button className="btn popup-btn" onClick={() => handleRemoveConfirm()}>Confirm Remove</button>
              </>
            }
            // footer={

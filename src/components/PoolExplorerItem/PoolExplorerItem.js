            ...
            <div>
                <div className="poolExplorer__box">
                    ...
                    <div className="poolExplorer__box_pair">
                        <img className="poolExplorer__icon_margin" src={iconGenerator(props.pair.symbolA)}
                             alt={props.pair.symbolA}/>
                        <b>{props.pair.symbolA}</b>
                    </div>
                    <span className="poolExplorer__box_text">/</span>
                    <div className="poolExplorer__box_pair">
                        <img className="poolExplorer__icon_margin" src={iconGenerator(props.pair.symbolB)}
               ...
               <span className="select-item-descr"><div>
                 1 {props.pair.symbolB} = <b>{props.pair.rateBA < 0.0001 ? parseFloat(props.pair.rateBA).toFixed(8) : parseFloat(props.pair.rateBA).toFixed(4)}</b> {props.pair.symbolA}
                    ...
                    <div className="poolExplorer__fixed_width">
                        <div className="poolExplorer__reserve">
                            <img className="poolExplorer__icon" src={iconGenerator(props.pair.symbolA)}
                                 alt={props.pair.symbolA}/>
                            {Number((parseFloat(props.pair.reserveA) / 1e9).toFixed(4)).toLocaleString('ru-RU')}
                        </div>
                        <div className="poolExplorer__reserve">
                            <img className="poolExplorer__icon" src={iconGenerator(props.pair.symbolB)}
                                 alt={props.pair.symbolB}/>
                            {Number((parseFloat(props.pair.reservetB) / 1e9).toFixed(4)).toLocaleString('ru-RU')}
                        </div>
                    </div>

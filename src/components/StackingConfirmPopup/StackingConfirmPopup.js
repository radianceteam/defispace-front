        ...
        content={
          <>
            ...
            <div className="confirm-block swap-confirm-block">
              <span className="confirm-token"><img className="confirm-icon" width={35} height={28} src={iconGenerator("STACKING")} alt={"STACKING"}/>{props.stake || 0}</span>
              { appTheme === 'light' ?
              ...
              <span className="confirm-token">{props.program.name || "Error Term"}</span>
            </div>
            <p className="confirm-text" style={{    fontWeight: "500"}}>
              After staking into this deposit program you <strong>{props.program.period === 0 ? `don't` : `will`}</strong> receive a TrueNFT deposit certificate token that can be used as collateral to get liquidity before the end of term or simply sold on an auction.
            </p>
            <button className="btn popup-btn" onClick={() => handleSwap()}>Confirm Staking</button>
          </>
        }
        footer={
            ...
            <Stack spacing={1}>
              <Stack direction={"row"} sx={{justifyContent: "space-between"}}>
                <div className="swap-confirm-wrap">
                  <p className="mainblock-footer-value">{props.program.period} months</p>
                  <p className="mainblock-footer-subtitle">Program duration</p>
                </div>
                <div className="swap-confirm-wrap">
                  <p className="mainblock-footer-value end">{props.program.apy || 11}%</p>
                  <p className="mainblock-footer-subtitle">APY</p>
                </div>
              </Stack>
              <Stack direction={"row"} sx={{justifyContent: "space-between"}}>
                <div className="swap-confirm-wrap">
                  <p className="mainblock-footer-value"><img className="confirm-icon" width={24} height={19} src={iconGenerator("STACKING")} alt={"STACKING"}/>{Number(props.profit).toFixed(1)}</p>
                  <p className="mainblock-footer-subtitle">Your profit</p>
                </div>
                <div className="swap-confirm-wrap">
                  <p className="mainblock-footer-value end"><img className="confirm-icon" width={24} height={19} src={iconGenerator("STACKING")} alt={"STACKING"}/>{(Number(props.stake) + Number(props.profit)).toFixed(1)}</p>
                  <p className="mainblock-footer-subtitle">You will have <br/>in {props.program.period} months</p>
                </div>
              </Stack>
            </Stack>
          </div>

...
function OrdersInput(props) {
    ...
    const swapFromSelectIsVisible = useSelector(state => state.limitOrders.ordersFromSelectIsVisible);
    const swapToSelectIsVisible = useSelector(state => state.limitOrders.ordersToSelectIsVisible);
    ...
    async function handleClick() {
        try {
            if (props.type === 'to' && !swapFromToken.symbol) {
                ...
                dispatch(props.type === 'from' ? showOrdersFromSelect() : showOrdersToSelect());
            }
        } catch (e) {
            ...
            `}</span>
                ...
                <div className="input-wrapper">
                    ...
                    {!props.token.symbol ? (
                        ...
                        <>
                            {(walletIsConnected && props.type === 'from') && <button className="input-max"
                                                                                     onClick={() => setValue(parseFloat(props.token.balance).toFixed(4))}>MAX</button>}
                            <button className="input-select" onClick={() => handleClick()}>
                                <img src={iconGenerator(props.token.symbol)} alt={props.token.symbol}

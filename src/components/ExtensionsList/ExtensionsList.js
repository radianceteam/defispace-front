...
function ExtensionsList() {
  ...
  const visibleEnterSeedPhrase = useSelector(state => state.enterSeedPhrase.enterSeedPhraseIsVisible);

  console.log(787, visibleEnterSeedPhrase)
  // useEffect(async ()=>{
  //   try {
            ...
            <div className="extensions-list">
              <div className="extensions-list-item"  onClick={() => handleClick("mnemonic")}>
                <div style={{
                  ...
                  <button style={{fontSize: "16px", width: "100%", marginBottom: "40px"}} className="btn wallet-btn" onClick={() => dispatch(showEnterSeedPhrase())}>Login using Seed Phrase</button>
                  <button style={{fontSize: "16px", width: "100%"}} className="btn wallet-btn" onClick={() => dispatch(showEnterSeedPhraseRegister())}>Create a new Seed Phrase and Wallet</button>
                  {visibleEnterSeedPhrase === true && <EnterSeedPhrase/>}
